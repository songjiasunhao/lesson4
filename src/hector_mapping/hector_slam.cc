    ...
    : private_node_("~"), lastGetMapUpdateIndex(-100), tfB_(0), map_publish_thread_(0)
{
   ...
   point_cloud_subscriber_ = node_handle_.subscribe(p_scan_topic_, p_scan_subscriber_queue_size_, &HectorMappingRos::scanCallback, this); // 雷达数据处理
...
void HectorMappingRos::InitParams()
{
    ...
    private_node_.param("scan_topic", p_scan_topic_, std::string("lslidar_point_cloud"));
    ...
    private_node_.param("map_size", p_map_size_, 8192);
 ...
 * 算法中所有的计算都是在地图尺度下进行。  
 */
void HectorMappingRos::scanCallback(const sensor_msgs::PointCloud2::ConstPtr &pointcloud)
{
   ...
   //滤波只留下一点
    ...
    pass.setFilterLimits(-0.1, 0.1);         //指定滤波范围
    pass.setNegative(false);                 //true:过滤掉指定范围的点，保留范围外的点；false时相反
    pass.filter(*cloud);           //执行滤波，返回滤波后的数据

    pcl::toROSMsg(*cloud,pointcloud2_);

    //将Pointcloud2转为Pointcloud
    sensor_msgs::convertPointCloud2ToPointCloud(pointcloud2_,pointcloud_);
    ...
    if (rosPointCloudToDataContainer(pointcloud_, laserTransform_, laserScanContainer, slamProcessor->getScaleToMap()))
    {
        // 首先获取上一帧的位姿，作为初值
    ...
    poseInfoContainer_.update(slamProcessor->getLastScanMatchPose(), slamProcessor->getLastScanMatchCovariance(), pointcloud->header.stamp, p_map_frame_);
    ...
    if (pub_map_to_baselink_tf_)
    {
        ...
        {
            tfB_->sendTransform(tf::StampedTransform(map_to_odom_, pointcloud->header.stamp, p_map_frame_, p_odom_frame_));
            tfB_->sendTransform(tf::StampedTransform(poseInfoContainer_.getTfTransform(), pointcloud->header.stamp, p_odom_frame_, p_tf_map_scanmatch_transform_frame_name_));
        ...
        {
            tfB_->sendTransform(tf::StampedTransform(poseInfoContainer_.getTfTransform(), pointcloud->header.stamp, p_map_frame_, p_tf_map_scanmatch_transform_frame_name_));
        }
    }
